...
void test_i2c(void)
{
	...
	while(1)
	{
		...
		xfer.txSz = 2;
		xfer.rxSz = 0;
		txBuff[0] = 0; // select register 0
		txBuff[1] = 0; // select latch acceleration data
		xfer.rxBuff = &rxBuff[0];
		acc_io.retcodeTransfer = Chip_I2C_MasterTransfer(i2cDev, &xfer);

		xfer.txSz = 1;
		txBuff[0] = 1; // select register 1
		xfer.rxSz = 4; // expect 4 data back: XH, XL, YH, YL
		xfer.rxBuff = &rxBuff[0];
		xfer.txBuff = &txBuff[0];
		memset(rxBuff, 0, sizeof(rxBuff));
		acc_io.retcodeSend =  Chip_I2C_MasterTransfer(i2cDev, &xfer);

		{
			uint16_t Xacc = rxBuff[0];
			Xacc <<= 8;
			Xacc |=  rxBuff[1];
			acc_io.Xacc = Xacc;
		}
		{
			uint16_t Yacc = rxBuff[2];
			Yacc <<= 8;
			Yacc |=  rxBuff[3];
			acc_io.Yacc = Yacc;
		}
